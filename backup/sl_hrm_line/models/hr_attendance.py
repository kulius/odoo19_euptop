...
class HrAttendance(models.Model):
    ...
    def get_today_status(self, employee_id):
        ...
        if attendance:
            ...
            check_out_local = attendance.check_out.replace(tzinfo=pytz.UTC).astimezone(tz) if attendance.check_out else None
    ...
    def clock_action(self, employee_id, clock_type, latitude=None, longitude=None, accuracy=None):
        ...
        :param accuracy: GPS 精確度 (目前未使用，Odoo 17 無此欄位)
        :return: dict
        """
        tz = pytz.timezone('Asia/Taipei')
        now = datetime.now(tz)
    ...
    def get_attendance_history(self, employee_id, limit=30, offset=0):
        ...
        for att in attendances:
            ...
            check_out_local = att.check_out.replace(tzinfo=pytz.UTC).astimezone(tz) if att.check_out else None
