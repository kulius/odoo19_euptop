    ...
    def calc_personal_calendar_time(self, employee_id, request_from, request_to):
        ...
        if not employee_id.schedule_id.is_user_personal_calendar:
            ...
            while current < end:
                ...
                # _logger.info("星期%s班表" % current.weekday(), worktime)
                if worktime:
                    ...
                    if shifts:
                        for shift in shifts:
                            ...
                            if overlapping_holidays:
                                ...
                                # 花了幾個小時想了各種情況(多公眾假日重疊，開始/結束時間剛好跟工作時間錯開等)
                                # 乾脆把每個區間分成15分鐘，然後檢查每個15分鐘的區間是否有假日再逐一扣掉
                                # 因為系統最低也是以15分鐘再請假
                                # 還有設定公眾假日也不太可能出現各種亂七八糟的分秒，
                                # 所以用這笨方法，簡單明瞭，自動就解決重疊問題
...
class HrScheduleWorktime(models.Model):
    ...
    def create(self, vals):
        if not vals['name']:
            ...
            time_seconds_start = int(vals['work_start'] * 3600)
            time_obj_start = time(hour=(time_seconds_start // 3600), minute=(time_seconds_start % 3600) // 60)
            time_seconds_end = int(vals['work_end'] * 3600)
