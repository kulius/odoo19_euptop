...
export class ActivityMenu extends Component {
    ...
    static props = [];
    static template = "sl_hr_attendance.attendance_menu";
    ...
    async searchReadEmployee(){
        const result = await this.rpc("/sl_hr_attendance/attendance_user_data");
        this.employee = result;
        if (this.employee.id) {
            this.hoursToday = this.date_formatter(
                this.employee.hours_today
            );
//            console.log(this.employee.attendance.check_in)
            this.attendance_date = this.employee.attendance.check_in_date
            this.check_in = this.employee.attendance.check_in;
            this.check_out = this.employee.attendance.check_out;
...
registry
    .category("systray")
    .add("sl_hr_attendance.attendance_menu", systrayAttendance, { sequence: 101 });
