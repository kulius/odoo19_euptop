...
class StarryLordHrAutoJob(models.TransientModel):
    ...
    def job_run(self):
        # find out tdoay resign employee
    ...
    def job_auto_create_on_board_user(self):
        ...
        domain = [('active', '=', True), ('user_id', '=', False), ('work_email', '!=', False), ] #('state', '=', 'working'), ('registration_date', '=', datetime.date.today())
        all_employee_ids = self.env['hr.employee'].search(domain)
        # onboard_employee_ids = all_employee_ids.filtered(lambda employee: )
