...
class HrEmployeeInheritance(models.Model):
    ...
    # def get_default_labor_refund_ratio(self):
    #     setting = self.env['ir.config_parameter'].sudo()
    #     labor_pension_rate = float(setting.get_param('labor_pension_rate', default=False)) / 100  # 勞退費率
    #     self.company_labor_refund_ratio = str(labor_pension_rate)
    ...
    def create(self, vals):
        ...
        data = super(HrEmployeeInheritance, self).create(vals)

        # 設定附件為 public
        for rec in self.id_card_ids:
            rec.sudo().public = True
        for rec in self.driving_license_ids:
            rec.sudo().public = True
        for line in self.health_report_line_ids:
            line.health_report_upload.sudo().public = True
        for line in self.license_line_ids:
    ...
    def write(self, vals):
        ...
        for line in self.license_line_ids:
            line.license_upload.sudo().public = True
        # for line in self.labor_pension_history_ids:
        #     line.attachment_ids.sudo().public = True
        # for line in self.labor_insurance_history_ids:
        #     line.attachment_ids.sudo().public = True
        # for line in self.health_insurance_history_ids:
        #     line.attachment_ids.sudo().public = True
        # for line in self.labor_harm_insurance_history_ids:
        #     line.attachment_ids.sudo().public = True
        # for line in self.insurance_doc_line_ids:
        #     line.file_upload.sudo().public = True
        return data
    ...
    # def on_board_state_compute(self):
    #     for rec in self:
    #         employee_working = rec.hr_employee_working_ids.sorted('start_date', reverse=True)[:1]
    #         if employee_working.change_reason_selection == 'on_board':
    #             rec.on_board_state = 'on_board'
    #         elif employee_working.change_reason_selection == 'resignation':
    #             rec.on_board_state = 'resignation'
    #         else:
    #             rec.on_board_state
...
class ResPartnerBank(models.Model):
    ...
    book_cover = fields.Many2many(
        "ir.attachment", string="銀行存摺封面", inverse_name="res_partner_bank_ids"
    )

    @api.model
    def create(self, vals_list):
        data = super(ResPartnerBank, self).create(vals_list)
        for rec in data.book_cover:
            rec.sudo().public = True
        return data
