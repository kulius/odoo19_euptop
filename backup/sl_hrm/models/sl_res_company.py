...
class StarryLordResCompany(models.Model):
    ...
    employee_number_prefix = fields.Char(string="員工編號前綴", required=True, help="員工編號前綴")
    quotation_prefix = fields.Char(string="報價單前綴", required=True, help="報價單前綴")
...
class StarryLordCompanySite(models.Model):
    ...
    company_id = fields.Many2one(comodel_name="res.company", inverse_name="company_site_line_ids", string="公司別")
...
class StarryLordWorkLocation(models.Model):
    ...
    def _compute_display_name(self):
        # self.display_name = '%s - %s' % (self.company_id.name, self.name)
        result = []
        for rec in self:
