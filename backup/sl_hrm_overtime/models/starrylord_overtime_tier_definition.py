...
class StarryLordOvertimeApplyValidation(models.Model):
    _name = 'starrylord.overtime.apply'
    _inherit = ['starrylord.overtime.apply', 'tier.validation']
    _description = '加班申請'
    ...
    def _compute_is_readonly(self):
        ...
        for record in self:
            ...
            #     can_edit = True
            if self.state in ['f_approve'] and self.can_review:
                is_readonly = False
    ...
    def to_draft(self):
        super(StarryLordOvertimeApplyValidation, self).restart_validation()
        super(StarryLordOvertimeApplyValidation, self).to_draft()

    def to_f_approve(self):
        super(StarryLordOvertimeApplyValidation, self).to_f_approve()
        self.request_validation()

    def to_cancel(self):
        super(StarryLordOvertimeApplyValidation, self).to_cancel()

    def _validate_tier(self, tiers=False):
        super(StarryLordOvertimeApplyValidation, self)._validate_tier(tiers)
        ...
        if self.validated:
            ...
            self.env['mail.message'].create({
                ...
                'body': f"加班單 [{self.name}] 已通過審核。",
                'subject': "加班單審核通過通知",
                'record_name': '加班單審核通過通知 ' + self.name,
    ...
    def _rejected_tier(self, tiers=False):
        super(StarryLordOvertimeApplyValidation, self)._rejected_tier(tiers)
        # 如果查無任何pending(待辦)的tier, 則直接拒絕(refused)
        if self.rejected:
            self.to_refused()
            # 發送通知給申請人
            # group_xml_id = 'sl_account.group_sl_payment_record_manager'  # 替换为你的群组XML ID
            ...
            self.env['mail.message'].create({
                'message_type': "notification",
                'body': f"加班單 [{self.name}] 被退回<br/>意見：{comment if comment else ''}。",
                'subject': "加班單退回通知",
                'record_name': '加班單退回通知 ' + self.name,
                'partner_ids': [(4, user.partner_id.id) for user in self.env['res.users'].browse(user_ids)],
                'model': self._name,
                'res_id': self.id,
                'notification_ids': notification_ids,
...
class TierDefinition(models.Model):
    ...
    def _get_tier_validation_model_names(self):
        ...
        res.append("starrylord.overtime.apply")
        return res
