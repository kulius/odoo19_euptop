...
class SaleOrder(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('pricelist_id', ''):
                ...
                shopify_instance_id = self.env['shopify.instance'].sudo().search([('id', '=', vals['shopify_instance_id'])], limit=1)
                if pricelist_id.currency_id.name != shopify_instance_id.shop_currency:
                    raise UserError(_("The Pricelist currency and Shopify currency does not match. \n\nPlease update the pricelist currency or authenticate Shopify instance again!!!"))
    ...
    def get_latest_shopify_id(self):
        latest_order = self.env['sale.order'].search([], order='id desc', limit=1)
        if latest_order:
            if latest_order.shopify_id:
                return latest_order.shopify_id
        return False
        
    def cron_import_shopify_orders(self):
        ''' * **Order import from shopify using cron** '''
        if not self.env['sale.order.import.tracking'].is_import_completed() :
            if not self.env['sale.order.import.tracking'].is_cool_off_minutes_passed() :
                _logger.info("\n\n\nSale Order Import in progress, cron job skipped.\n\n")
                return
        ...
        if not cron_lock:
            cron_lock = self.env['shopify.cron.job.lock'].create({'name': 'cron_import_shopify_orders', 'is_running': True})
        else:
            cron_lock.is_running = True
        try:
            # Fetching last 10 days orders only       
            shopify_instance_id = self.env["shopify.instance"].sudo().search([], limit=1)
            for order in self.get_all_orders(shopify_instance_id,  order_status = "open", days_to_import=10):
                self.create_sale_order(order, shopify_instance_id, )
            return

        finally:
            cron_lock.is_running = False
         
    def get_all_orders(self, instance_id, order_status = "open", days_to_import = False, limit=100):
        session = self.init_shopify_session(instance_id)
        shopify.ShopifyResource.activate_session(session)
        
        if days_to_import:
            today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
            start_date = today - timedelta(days=days_to_import)
            start_date_str = start_date.strftime('%Y-%m-%dT%H:%M:%S')
        
            if self.env['shopify.order.import.time'].is_imported():
                order_import_time = self.env['shopify.order.import.time'].get_import_time()
                
                if order_import_time:
                    # order_import_date = order_import_time.replace(hour=0, minute=0, second=0, microsecond=0)  
                    order_import_date = max(start_date, order_import_time)
                    start_date_str = order_import_date.strftime('%Y-%m-%dT%H:%M:%S')            
        
        current_import = self.env['sale.order.import.tracking'].sudo().search([], limit=1)
        
        count = 0
        get_next_page = True
        if not current_import:
            since_id = 0
            self.env['sale.order.import.tracking'].create_or_update_record({'since_id': '0','is_completed': False,'status': 'start'})
            total_count = shopify.Order.count(status = order_status)
            self.env['sale.order.import.tracking'].update_total_count(total_count)            
        else:
            since_id = current_import.since_id
            
        while get_next_page:
            if days_to_import:
                orders = shopify.Order.find(created_at_min = start_date_str, since_id=since_id, limit=limit, status=order_status)
            else:
                orders = shopify.Order.find(since_id=since_id, limit=limit, status=order_status)
            ...
            self.env['sale.order.import.tracking'].create_or_update_record({'since_id': since_id,'is_completed': False,'status': 'in_progress'})
            self.env['sale.order.import.tracking'].update_count(limit)
            ...
            if len(orders) < limit :
                ...
                # set first import time in db
                if not self.env['shopify.order.import.time'].is_imported():
                    self.env['shopify.order.import.time'].set_order_import_time()
                
            if count >= 10000:
                get_next_page = False
                self.env['sale.order.import.tracking'].mark_as_completed()
                     
    def import_sale_order(self, instance_id, order_status = "open"):
        # Fetching two months old orders
        for order in self.get_all_orders(instance_id, order_status = order_status, days_to_import=60):
    ...
    def create_sale_order(self, order, instance_id):
        ...
        if order.customer:
            ...
            _logger.info('\n\n\n\n  customer not exist for order  =  %s \n\n\n\n' % (order.id) )
        
        if not res_partner and order.customer:
        ...
        if res_partner:
            ...
            if order.created_at and len(order.created_at) > 19:
                dict_so['date_order'] = datetime.strptime(order.created_at[:19], '%Y-%m-%dT%H:%M:%S')
            else:
                dict_so['date_order'] = order.created_at

            item_exist = self.check_sale_order_line_products_exist(order)           
            if not item_exist:
                _logger.info('\n\n\n\n  Skipping the import of this order because one or more product items do not exist in the current product list.  =  %s \n\n\n\n' % (order.id) )
                return
                    ...
                    for sol_item in order.line_items:
                        ...
                        if res_product:
                            ...
                            if s_order_line:
                                ...
                                so_line = self.create_sale_order_line(sale_order.id, instance_id, order, sol_item)
                        self.env.cr.commit()
    ...
    def check_sale_order_line_products_exist(self, order):
        for sol_item in order.line_items:
            res_product = ''     
            if sol_item.product_id or sol_item.variant_id:
                res_product = self.env['product.product'].sudo().search(
                    ['|', ('shopify_variant_id', '=', sol_item.product_id), ('shopify_variant_id', '=', sol_item.variant_id)],
                    limit=1)
        ...
        if tax_lines:
            for tax_line in tax_lines:
                ...
                if acc_tax:
                    ...
                    dict_tax['invoice_label'] = shopify_tax_name
    ...
    def action_shopify_order_wizard(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id("mt_odoo_shopify_connector.action_shopify_order_actions_wizard")
        action.update({
    ...
    def action_generate_invoice(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id("mt_odoo_shopify_connector.action_wizard_generate_invoice")
        action.update({
...
class ShopifyOrderImportTime(models.Model):
    _name = 'shopify.order.import.time'
    _description = 'Shopify Order Import Time'

    name = fields.Char('Name', default='order_import_time')
    order_import_time = fields.Datetime('First Import Time')
    
    
    def is_imported(self):
        record = self.search([('name', '=', 'order_import_time')], limit=1)
    ...
    def set_order_import_time(self):
        record = self.search([('name', '=', 'order_import_time')], limit=1)

        if not record:
            self.create({'order_import_time': fields.Datetime.now()})
            
    def get_import_time(self):
        record = self.search([('name', '=', 'order_import_time')], limit=1)
        if record:
            return record.order_import_time
        else:
            return False