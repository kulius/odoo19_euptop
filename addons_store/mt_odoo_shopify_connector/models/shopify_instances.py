...
class ShopifyInstance(models.Model):
    ...
    api_version = fields.Char(string="API Version", required=True, store=True,  help="API Version", default="2025-01")
    dashboard_graph_data = fields.Text(compute='_kanban_dashboard_graph')
    shop_currency = fields.Char(string="Shopify Currency")
    ...
    def write(self, vals):
        ...
        if vals.__contains__('shop_url') or vals.__contains__('admin_api_key') or vals.__contains__('api_version'):
            vals['is_authenticated'] = False

        super(ShopifyInstance, self).write(vals)
        self.env.cr.commit()  
    ...
    def _kanban_dashboard_graph(self):
        for rec in self:
            # Use context with default sort if not provided
            sort_value = rec._context.get('sort', 'month')
            rec_with_context = rec.with_context(sort=sort_value)
            values = rec_with_context.get_dashboard_data_shopify(rec)
            sales_total = round(sum([key['y'] for key in values]), 2)
            shopify_orders_data = rec_with_context.get_total_orders()
    ...
    def get_dashboard_data_shopify(self, record):
        ...
        sort_value = self._context.get('sort', 'month')
        if sort_value == 'week':
    ...
    def create_action(self, view, domain):
        ...
        # Convert 'tree' view type to 'list' in Odoo 19
        # Convert view_mode string (e.g., "tree,form" -> "list,form")
        if action.get('view_mode'):
            action['view_mode'] = action['view_mode'].replace('tree', 'list')
        
        # Convert 'tree' view type to 'list' in views list
        if action.get('views'):
            new_views = []
            for view_tuple in action['views']:
                if isinstance(view_tuple, (list, tuple)) and len(view_tuple) >= 2:
                    view_id, view_type = view_tuple[0], view_tuple[1]
                    if view_type == 'tree':
                        new_views.append((view_id, 'list'))
                    else:
                        new_views.append(view_tuple)
                else:
                    new_views.append(view_tuple)
    ...
    def get_total_orders(self):
        ...
        view = self.env['ir.actions.act_window']._for_xml_id('mt_odoo_shopify_connector.action_sale_order_tree_shopify')
    ...
    def get_products(self):
        ...
        self._cr.execute(
            """select count(id) as total_count from product_template where active = True and shopify_instance_id = %s""" % self.id)
        result = self._cr.dictfetchall()

        if result:
            total_count = result[0].get('total_count')

        view = self.env['ir.actions.act_window']._for_xml_id('sale.product_template_action')
    ...
    def get_customers(self):
        ...
        view = self.env['ir.actions.act_window']._for_xml_id('account.res_partner_action_customer')
        action = self.create_action(view, [('shopify_instance_id', '=', self.id)])
        action.update({
