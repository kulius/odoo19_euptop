    ...
    def cron_import_shopify_customers(self):
        if not self.env['customer.import.tracking'].is_import_completed() :
            if not self.env['customer.import.tracking'].is_cool_off_minutes_passed() :
                _logger.info("\n\n\nCustomer Import in progress, cron job skipped.\n\n")
                return 
        ...
        if not cron_lock:
            cron_lock = self.env['shopify.cron.job.lock'].create({'name': 'cron_import_shopify_customers', 'is_running': True})
        else:
            cron_lock.is_running = True
        try:
            shopify_instance_id = self.env["shopify.instance"].sudo().search([], limit=1)
            self.env['res.partner'].import_customer(shopify_instance_id)
            return
