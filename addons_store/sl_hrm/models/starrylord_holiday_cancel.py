    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('name') and vals.get('holiday_apply_id'):
                apply_rec = self.env['starrylord.holiday.apply'].browse(vals['holiday_apply_id'])
                count = self.search_count([
                    ('holiday_apply_id', '=', apply_rec.id)
                ]) + 1
                vals['name'] = f"{apply_rec.name}-銷假單-{count}"
...
class StarryLordHolidayUsedRecordCancel(models.Model):
    ...
    is_cancel_record = fields.Boolean(
        string='是否為銷假沖銷',
        compute='_compute_is_cancel_record',
        store=True
    )
