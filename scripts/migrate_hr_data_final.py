...
Final version - Matches actual Odoo 19 schema
"""
import psycopg2
import json
...
def extract_name_from_jsonb(name_val):
    """Extract string name from JSONB value"""
    if name_val is None:
        return None
    if isinstance(name_val, str):
        try:
            name_val = json.loads(name_val)
        except:
            return name_val
    if isinstance(name_val, dict):
        return name_val.get('zh_TW') or name_val.get('en_US') or list(name_val.values())[0] if name_val else None
    return str(name_val)
...
def migrate_departments(conn17, conn19):
    ...
    cur17.execute("""
        SELECT id, name, parent_id, manager_id, active
        FROM hr_department
        ORDER BY id
    """)
    ...
    id_mapping = {}
    for dept in depts:
        old_id, name_json, parent_id, manager_id, active = dept

        name = extract_name_from_jsonb(name_json)
        if not name:
            name = f'Department {old_id}'
        ...
        cur19.execute("""
            ...
            WHERE name::text LIKE %s OR name::text LIKE %s
        """, (f'%"{name}"%', f'%{name}%'))
        ...
        if existing_row:
            ...
            name_jsonb = json.dumps({'en_US': name, 'zh_TW': name})
            cur19.execute("""
                INSERT INTO hr_department (name, company_id, active, create_uid, write_uid, create_date, write_date)
                VALUES (%s::jsonb, %s, %s, 1, 1, NOW(), NOW())
                RETURNING id
            """, (name_jsonb, company_id, active if active is not None else True))
...
def migrate_employees(conn17, conn19, dept_mapping):
    ...
    for emp in employees:
        ...
        # Check if exists
        if emp_number:
            cur19.execute("SELECT id FROM hr_employee WHERE employee_number = %s", (emp_number,))
        else:
            cur19.execute("SELECT id FROM hr_employee WHERE name = %s", (name,))
        existing_row = cur19.fetchone()
        ...
        if existing_row:
            ...
            """, (emp_number, work_email, work_phone, mobile_phone,
                  gender, birthday, existing_row[0]))
            print(f"    -> 更新 (ID: {existing_row[0]})")
        else:
            # Create resource first (tz and time_efficiency are required in Odoo 19)
            cur19.execute("""
                INSERT INTO resource_resource (name, company_id, resource_type, tz, time_efficiency, active, create_uid, write_uid, create_date, write_date)
                VALUES (%s, %s, 'user', 'Asia/Taipei', 100.0, %s, 1, 1, NOW(), NOW())
                RETURNING id
            ...
            """, (name, emp_number, resource_id, company_id,
                  work_email, work_phone, mobile_phone,
                  gender, birthday, active if active is not None else True))
            new_id = cur19.fetchone()[0]
            id_mapping[old_id] = new_id
...
def migrate_annual_leave_settings(conn17, conn19):
    ...
    # Both Odoo 17 and 19 use: seniority, days, annual_period
    cur17.execute("""
        SELECT id, seniority, days, annual_period
    ...
    count = 0
    for s in settings:
        old_id, seniority, days, annual_period = s

        print(f"  處理特休: 年資 {seniority}, 天數 {days}")

        cur19.execute("SELECT id FROM starrylord_annual_leave_setting WHERE seniority = %s", (seniority,))
        if not cur19.fetchone():
            cur19.execute("""
                INSERT INTO starrylord_annual_leave_setting (seniority, days, annual_period, create_uid, write_uid, create_date, write_date)
                VALUES (%s, %s, %s, 1, 1, NOW(), NOW())
            """, (seniority, days, annual_period))
            count += 1
