...
Version 3 - Handle JSONB and encoding properly
"""
import psycopg2
...
def migrate_departments(conn17, conn19):
    ...
    for dept in depts:
        ...
        # Check if exists by checking JSON contains
        cur19.execute("""
        ...
        if existing_row:
            ...
            cur19.execute("""
                ...
                VALUES (%s::jsonb, %s, %s, %s, 1, 1, NOW(), NOW())
                RETURNING id
            """, (name_jsonb, complete_name or name, company_id, active if active is not None else True))
